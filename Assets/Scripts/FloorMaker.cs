...
//	Declare a private integer called myCounter that starts at 0; 		// count how many floor tiles this FloorMaker has instantiated
	...
	public static List<Transform> makerList = new List<Transform>();
...
//	Declare a public Transform called floorMakerPrefab, assign the prefab in inspector; 
	...
	void Start()
	{
		makerList.Add(floorMakerPrefab);
	}
	void Update () 
	{
		//	If counter is less than 50 and grandTotal is less than 500, then:
		if (myCounter < 50 && grandTotal < 500) {
			//Debug.Log("tiles: " + myCounter + " Total: " + grandTotal);
			...
			//	... Else if number is 0.99f-1.0f, then instantiate a floorMakerPrefab clone at my current position;
				...
				Transform maker = Instantiate(floorMakerPrefab, transform.position, Quaternion.Euler(0, 0, 0));
				makerList.Add(maker);
			}
			//  end elseIf
			//	Instantiate a floorPrefab clone at current position;
			Instantiate(floorPrefab, transform.position, Quaternion.Euler(0, 0, 0));
			//	Move 1 unit "upwards" based on this object's local rotation 
			//	(e.g. with rotation 0,0,0 "upwards" is (0,1,0)... but with rotation 0,0,180 then "upwards" is (0,-1, 0)... )
			transform.position += transform.up;
			//	Increment counter;
			myCounter++; grandTotal++;
		} else {
			//	Else:
